...
function setupEvents(g, selection, raise) {
  ...
  selection.on('click.zoom', function (d) {
    let curr = d3.select(this).datum().data;
    linkParams.name = curr.name;
    linkParams.depth += d3.select(this).datum().depth;
    drawNodeLinkVis(stratified);
  });

  selection.filter(function (d) {
    let curr = d3.select(this).datum().data;
    return curr.name == linkParams.name;
  }).on('click.zoom', function (d) {
    // selectedSelection.on('click.zoom', function(d) {
    let curr = d3.select(this).datum().data;
    linkParams.name = curr.parent;
    if (linkParams.name === '') { 
      linkParams.name = "All Incidents" 
    }
    linkParams.depth -= d3.select(this).datum().depth + 1;
    if (linkParams.depth < 0) { 
      linkParams.depth = 0 
    };
    drawNodeLinkVis(stratified);
  });
